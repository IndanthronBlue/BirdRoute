...
- 登录账号后，行程保存到后端 SQLite 数据库；完整行程内容仍以 JSON blob 保存，方便导入、导出和高级编辑。
...
BirdRoute 不再依赖前端内置鸟名表。鸟名映射由后端全局维护，当前保存在 SQLite 数据库中：

```text
backend_data/birdroute.sqlite
```
...
API Key 会保存到后端 SQLite 数据库，并通过后端密钥加密。不要把 API Key 写入 `config.js`、README 或公共模板文件。
...
旧版 `users.json`、`bird_name_mappings.json`、用户目录下的 `trips/*.json`、设置文件和缓存文件会在首次启动时自动迁移到 SQLite。旧文件不会自动删除，可作为迁移后的备份。
...
- `birdroute.sqlite` 保存用户、行程、默认行程、用户设置、API Key 密文、鸟名映射、eBird 缓存和坐标缓存。
- 行程内容在 SQLite 中仍以完整 JSON blob 保存，因此导入导出和“编辑当前行程 JSON”功能保持兼容。
- `research/`、浏览器配置和迁移前旧 JSON 文件仍可能存在于 `backend_data/users/<username>/`，不要手动删除未备份的数据。
...
可以。新版后端会在首次启动时把旧 JSON 数据迁移到 `backend_data/birdroute.sqlite`，包括用户、行程、默认行程、鸟名映射、用户设置和缓存。部署新版前仍建议备份整个 `backend_data/`，尤其是用户行程、鸟名映射、API Key 加密密钥和设置文件。
